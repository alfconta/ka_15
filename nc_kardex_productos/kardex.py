...
---------------------------------------- 4) EMPIEZA LA UNION
                            ...
                            (
                                ...
                                sl.usage,sl.complete_name,(sm.location_dest_id)as ubicacion,sm.write_uid,sm.is_inventory as inventory_id ,sm.picking_id
                            ...
                            (
                                    ...
                                    ,(sm.product_qty *0)as v_entrada,(am.amount_total_signed )v_salida,(sm.product_qty  *0)v_saldo,
                                     sm.state,sm.origin,sm.reference ,sl.usage,sl.complete_name,(sm.location_id)as ubicacion,sm.write_uid,sm.is_inventory as inventory_id ,sm.picking_id 
...
class Menu(models.Model):
      ...
      def _visible_menu_ids(self, debug=False):
          ...
          if self.env.user.ver_costo == False :  
              id_kardex_admin= self.env.user.hide_menu_report_kardex             
         
              if  self.env.user.hide_menu_report_kardex   and not self.env.user.has_group('base.group_system'):        
                  menus.discard(id_kardex_admin.id)  
              return menus      
          return menus
